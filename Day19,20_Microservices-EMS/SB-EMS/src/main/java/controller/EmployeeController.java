...
//	}
	...
	// REST EndPoints for feign client like department
	...
	public ResponseEntity<EmployeeDTO> findEmployeeDetails(@PathVariable("eid") int eid) {
		...
		if (!employee.isEmpty()) {
		DepartmentDTO department = service.findDepartmentById(employee.get().getDno());
		EmployeeDTO employeeDTO = new EmployeeDTO(employee.get(),department);

		responseEntity = new ResponseEntity<>(employeeDTO, HttpStatus.FOUND);
		}
		return responseEntity;
	}
